"""Regression tests for LM Studio base_url provider ownership (#1527/#1530)."""
...
def _mock_models_endpoint(monkeypatch, model_id: str = "qwen3-27b") -> None:
    ...
    monkeypatch.setattr(
        "socket.getaddrinfo",
        lambda *a, **k: [
            (socket.AF_INET, socket.SOCK_STREAM, 6, "", ("192.168.1.22", 0))
        ],
    )
...
def test_lmstudio_lan_ip_selection_resolves_to_configured_base_url(
...
""",
    ...
    assert not _models_for_provider(result, "ollama")
